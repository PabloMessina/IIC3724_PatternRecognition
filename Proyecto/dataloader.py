...
def random_crop(img, ratio, n_crops = 10):
    list_crops = []
    ...
    for i in range(n_crops):
        ...
        list_crops.append(resized_crop)
    return list_crops


def erase_background(m, name = 'filename.jpg'):
...
def create_dataset(root="FaceMask166", methods=['lbp'], split='A', full_face=True, path_load = 'features_split_{}_ff_{}_{}.pkl', crop_enable = False, n_crops = 10, ratio = 0.5):
    ...
    path_train_load = path_load.format(split, full_face, 'train', methods)
    x_train, gt_train, img_train = get_features_images(root, train_imgs, path_train_load, methods, full_face, crop_enable, n_crops, ratio, 'train')
    print("Features de Val")
    path_val_load = path_load.format(split, full_face, 'val', methods)
    x_val, gt_val, img_val = get_features_images(root, val_imgs, path_val_load, methods, full_face, crop_enable, n_crops, ratio, 'val')
    print("Features de Test")
    path_test_load = path_load.format(split, full_face, 'test', methods)
    x_test, gt_test, img_test = get_features_images(root, test_imgs, path_test_load, methods, full_face, crop_enable, n_crops, ratio, 'test')

    return x_train, gt_train, x_val, gt_val, x_test, gt_test, train_imgs, val_imgs, test_imgs, img_train, img_val, img_test


def get_features_images(root, imgs, path_load = None, methods = ['lbp', 'glcm', 'EV', 'GABOR', 'SEGL', 'HOG'], full_face = True, crop_enable = False, n_crops = 10, ratio = 0.5, part = 'train'):
    labels = []
    features = []
    images_t = []
    ...
    if path_load is not None and os.path.exists(path_load):
        ...
        for img in tqdm(imgs):
            ...
            if not full_face:
                img_info_woback = erase_background(img_info, img)
                img_info = cv2.resize(img_info_woback, (img_info.shape[1], img_info.shape[0])) 
            list_img.append(img_info)
            if crop_enable:
                crops = random_crop(img_info, ratio, n_crops)
                list_img.extend(crops)
            for e, img_s in enumerate(list_img):
                # cv2.imwrite(img.split(".")[0]+"_"+str(e)+'.'+img.split(".")[1], img_s)
                feature_img = compute_features(img_s, methods, full_face)
                features.append(feature_img)
                labels.append(class_id)
                # img_s = cv2.resize(img_s, (50, 50)) 
                images_t.append(img_s)
...
def compute_features(img, methods = ['lbp', 'glcm', 'EV', 'GABOR', 'SEGL', 'HOG'], full_face = True):
